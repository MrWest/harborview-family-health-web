...
function updateVisitorDraft(toolCall: ToolCall, actor: DemoActor): ToolResult {
  ...
  const patch = Object.entries(toolCall.parameters)
    .filter(
      ([field, value]) =>
        VISITOR_DRAFT_FIELD_SET.has(field) &&
        typeof value === "string" &&
        value.trim().length > 0,
    )
    .map(([field, value]) => [field, (value as string).trim()] as const)
  ...
  if (!patch.length)
    return blocked(
      "At least one approved, non-empty, user-confirmed registration value is required for a local draft update.",
    );

  const saved = JSON.parse(
    window.localStorage.getItem(VISITOR_DRAFT_STORAGE_KEY) ?? '{"fields":{}}',
  ) as { fields?: Record<string, string>; extractedFields?: string[] };
  const updatedFields = patch.map(([field]) => field);
  const next = {
    ...saved,
  ...
  window.dispatchEvent(
    new CustomEvent("harborview:visitor-draft-updated", { detail: next }),
  );
  ...
  return {
    ...
    detailed_data: {
      updatedFields,
      persisted: false,
      clientManagedBoundary: "browser-local visitor draft only",
    },
  };
}
...
export function createHarborviewAgentActions(
  options: AgentActionOptions,
): OnIntentDetected {
      ...
      switch (toolCall.toolName) {
        ...
        case "create_reception_intake_from_draft": {
          ...
          const intake = await clinicApi.createReceptionIntake(
            ...
            specialty,
            actor,
            typeof toolCall.parameters.selectedExistingPatientId === "string"
