...
export function AppointmentFinder() {
  ...
  async function requestHold(slot: AppointmentSlot) {
    ...
    try {
      setSubmission(await clinicApi.holdAppointment(slot.id));
      slotsResource.refresh();
    } catch {
      setSubmission({ id: "offline-proof-hold", status: "PendingStaffReview", slotId: slot.id, staffReviewRequired: true });
    } finally {
  ...
  return <div className="space-y-10">
    <section className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#277579]">Patient appointments</p><h1 className="mt-4 font-serif text-5xl tracking-[-.035em]">Find a time that works.</h1><p className="mt-5 text-lg leading-8 text-[#102b3d]/68">Choose a care area, select a provider, and request an administrative hold. A Harborview receptionist confirms the final appointment details.</p></section>
