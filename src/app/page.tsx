  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f7f7f2] text-[#102b3d]">
      ...
      <section className="relative isolate min-h-[720px] overflow-hidden bg-[#102b3d] text-white">
        ...
        <div className="relative mx-auto flex min-h-[720px] max-w-7xl items-center px-5 lg:px-8">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.26em] text-[#a9d9cf]">Harborview Family Health Centre</p>
            <h1 className="mt-6 font-serif text-5xl leading-[0.94] tracking-[-0.05em] sm:text-6xl lg:text-7xl">
      ...
      <section className="border-y border-[#102b3d]/10 bg-white">
        <div className="mx-auto grid max-w-7xl divide-y divide-[#102b3d]/10 px-5 sm:grid-cols-3 sm:divide-x sm:divide-y-0 lg:px-8">
          {[
            ["Patient-first", "A calm, administrative path from request to staff confirmation."],
            ["Reception-led", "One workspace for document completeness, holds, and practical follow-through."],
            ["Administrative only", "No diagnosis, triage, treatment recommendation, or medical decisioning."],
          ].map(([title, copy]) => (
            <div className="py-7 sm:px-7 sm:first:pl-0 sm:last:pr-0" key={title}>
              <p className="font-serif text-2xl tracking-[-0.025em]">{title}</p>
              <p className="mt-2 max-w-xs text-sm leading-6 text-[#102b3d]/62">{copy}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-24 lg:grid-cols-[0.85fr_1.15fr] lg:px-8 lg:py-32">
        <div className="lg:sticky lg:top-10 lg:self-start">
          ...
          <h2 className="mt-5 max-w-md font-serif text-5xl leading-[0.98] tracking-[-0.045em] sm:text-6xl">
            Less back-and-forth. <em className="font-normal text-[#277579]">More clarity.</em>
          </h2>
        ...
        <div className="space-y-4">
          {patientSteps.map(({ step, title, copy, icon: Icon }) => (
            <article className="group grid gap-5 rounded-[1.5rem] border border-[#102b3d]/10 bg-white p-6 transition hover:-translate-y-0.5 hover:border-[#277579]/35 sm:grid-cols-[70px_1fr_auto] sm:items-center sm:p-8" key={step}>
              <span className="font-serif text-4xl text-[#277579]/55">{step}</span>
              <div>
                <h3 className="font-serif text-3xl tracking-[-0.03em]">{title}</h3>
                <p className="mt-2 max-w-xl text-sm leading-6 text-[#102b3d]/65">{copy}</p>
              </div>
        ...
        <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-5 font-serif text-5xl leading-[0.98] tracking-[-0.045em] text-white sm:text-6xl">
              Built for the people who make care <em className="font-normal text-[#a9d9cf]">actually move.</em>
            </h2>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-white/67">
              The reception workspace turns a scattered administrative handoff into visible queues, explicit next actions, and staff-owned confirmation.
            </p>
          </div>

          <div className="mt-14 grid gap-4 lg:grid-cols-3">
            {receptionFlows.map(({ title, copy, href, icon: Icon }) => (
              <Link href={href} className="group flex min-h-72 flex-col rounded-[1.5rem] border border-white/15 bg-white/[0.06] p-7 backdrop-blur-sm transition hover:-translate-y-1 hover:border-[#a9d9cf]/55 hover:bg-white/[0.1]" key={title}>
                <Icon className="text-[#a9d9cf]" size={27} strokeWidth={1.6} />
                <h3 className="mt-auto font-serif text-3xl tracking-[-0.03em]">{title}</h3>
                <p className="mt-3 text-sm leading-6 text-white/63">{copy}</p>
                <span className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#a9d9cf]">Open flow <ArrowRight size={16} /></span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-24 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:py-32">
        <div className="relative min-h-[430px] overflow-hidden rounded-[2rem]">
          <Image src="/images/harborview-lab.webp" alt="Harborview laboratory coordination space" fill sizes="(min-width: 1024px) 52vw, 100vw" className="object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#102b3d]/78 via-transparent to-transparent" />
          <div className="absolute bottom-0 p-8 text-white"><p className="text-xs font-bold uppercase tracking-[.22em] text-[#a9d9cf]">One operating picture</p><p className="mt-3 max-w-sm font-serif text-3xl leading-tight">Appointments, documents, laboratory coordination, and staff review—not isolated tabs.</p></div>
        </div>
        <div className="flex flex-col justify-center">
          <SectionLabel>How Harborview operates</SectionLabel>
          <h2 className="mt-5 font-serif text-5xl leading-[0.98] tracking-[-0.045em] sm:text-6xl">Human confirmation is <em className="font-normal text-[#277579]">the point.</em></h2>
          <p className="mt-6 text-lg leading-8 text-[#102b3d]/68">The platform gives staff a better administrative handoff. It does not automate clinical judgment, make decisions for patients, or replace established care processes.</p>
          <div className="mt-9 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl bg-[#dcece7] p-5"><UsersRound className="text-[#277579]" size={24}/><h3 className="mt-5 font-serif text-2xl">One shared view</h3><p className="mt-2 text-sm leading-6 text-[#102b3d]/65">Patient and reception experiences remain connected through the same administrative workflow.</p></div>
            <div className="rounded-2xl bg-[#f0eee5] p-5"><ShieldCheck className="text-[#277579]" size={24}/><h3 className="mt-5 font-serif text-2xl">Clear boundaries</h3><p className="mt-2 text-sm leading-6 text-[#102b3d]/65">Every action is explicitly administrative, reviewable, and ready for staff follow-through.</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-[#102b3d]/10 bg-white px-5 py-12 text-center lg:px-8">
        <Brand />
        <p className="mt-5 text-sm text-[#102b3d]/55">© 2026 Harborview Family Health Centre · Fictional proof environment</p>
        <p className="mt-2 text-xs text-[#102b3d]/45">Administrative coordination only · Not medical advice</p>
      </section>
    </main>
  );
}
