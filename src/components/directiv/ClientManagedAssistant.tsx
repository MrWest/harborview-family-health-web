...
function slotItems(slots: Array<{ id: string; resourceId: string; startsAtUtc: string; endsAtUtc: string }>) {
  return slots.map((slot) => ({ id: slot.id, name: new Intl.DateTimeFormat("en-CA", { dateStyle: "medium", timeStyle: "short" }).format(new Date(slot.startsAtUtc)), type: `Provider ${slot.resourceId}`, price: 0, description: `Available through ${new Date(slot.endsAtUtc).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` }));
}
...
export function ClientManagedAssistant({ actor, conversationId, pageName }: AssistantProps) {
  ...
  const onIntentDetected = useMemo<OnIntentDetected>(() => async (toolCall: ToolCall): Promise<ToolResult> => {
    try {
