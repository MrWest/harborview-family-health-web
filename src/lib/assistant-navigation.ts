...
type RecordRoute = {
  actor: ActorMode;
  label: string;
  pathname: (recordId: string) => string;
};
...
export const ASSISTANT_VIEW_ROUTES: Record<NavigationDestination, ViewRoute> = {
  ...
  patient_results: {
    actor: "activePatient",
    label: "laboratory result status",
    pathname: "/patient/results",
  },
  patient_documents: {
    actor: "activePatient",
    label: "document requests",
    pathname: "/patient/intake",
  },
  reception_dashboard: {
    actor: "receptionist",
  ...
  reception_intake: {
    ...
    pathname: "/reception/intake",
  },
  reception_new_intake: {
    actor: "receptionist",
    label: "new intake",
    pathname: "/reception/new-intake",
  },
  reception_patients: {
    actor: "receptionist",
  ...
  reception_schedule: {
    actor: "receptionist",
    label: "schedule",
    pathname: "/reception/schedule",
  },
  reception_laboratory: {
    actor: "receptionist",
    label: "laboratory workspace",
    pathname: "/reception/laboratory",
  },
};

const encodedRecord = (recordId: string) => encodeURIComponent(recordId);

export const ASSISTANT_RECORD_ROUTES: Record<
...
> = {
  patient_appointment: {
    ...
    pathname: (id) => `/patient/appointments/${encodedRecord(id)}`,
  },
  laboratory_result_release: {
    actor: "activePatient",
    label: "laboratory release detail",
    pathname: (id) => `/patient/results/${encodedRecord(id)}`,
  },
  patient_document_request: {
    actor: "activePatient",
    label: "document request detail",
    pathname: (id) => `/patient/documents/${encodedRecord(id)}`,
  },
  registration_draft: {
    actor: "receptionist",
    label: "registration draft detail",
    pathname: (id) => `/reception/drafts/${encodedRecord(id)}`,
  },
  patient_profile: {
    actor: "receptionist",
    label: "patient administrative profile",
    pathname: (id) => `/reception/patients/${encodedRecord(id)}`,
  },
  intake_case: {
    actor: "receptionist",
    label: "recognition intake detail",
    pathname: (id) => `/reception/intakes/${encodedRecord(id)}`,
  },
  reception_appointment: {
    actor: "receptionist",
    label: "reception appointment detail",
    pathname: (id) => `/reception/appointments/${encodedRecord(id)}`,
  },
};

export function isNavigationDestination(
  value: string,
): value is NavigationDestination {
  return value in ASSISTANT_VIEW_ROUTES;
}

export function isNavigationRecordType(
  value: string,
): value is NavigationRecordType {
